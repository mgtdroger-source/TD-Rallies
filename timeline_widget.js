...
<div class="tlw-root" data-tlw-root="1">
  <div class="tlw-row">
    ...
    <div class="tlw-panel tlw-panel-timeline">
      <h2>Distance lanes (30 unit window)</h2>
...
// ---
    ...
    function computePositionsForRow(rowIndex){
      ...
      const positions = absTimes.map(entry => {
        ...
        const scanAbsMin = tRefOffset;

        // Remove any ADDED_TIME hold overlap between the scan time and the selected row's
        // absolute time, so groups do not jump backwards before a hold or get pinned at the
        // selected waypoint after they should have moved beyond it.
        let heldMinBetween = 0;
        for (let i = 0; i < rowIndex; i++) {
          const holdRow = rows[i];
          if (String(holdRow?.type || '') !== 'ADDED_TIME') continue;

          const addSec = _getAddedSecForRow(holdRow, entry.group);
          if (!(addSec > 0)) continue;

          const holdBaseMin = parseTime((holdRow.times || {})[entry.group]);
          if (!Number.isFinite(holdBaseMin)) continue;

          const holdStartAbsMin = holdBaseMin + (startDelayByGroup[entry.group] || 0);
          const holdEndAbsMin = holdStartAbsMin + (addSec / 60);

          const spanStart = Math.min(scanAbsMin, entry.absMin);
          const spanEnd = Math.max(scanAbsMin, entry.absMin);
          const overlapStart = Math.max(spanStart, holdStartAbsMin);
          const overlapEnd = Math.min(spanEnd, holdEndAbsMin);
          if (overlapEnd > overlapStart) heldMinBetween += (overlapEnd - overlapStart);
        }

        const dt = entry.absMin - scanAbsMin; // signed minutes from scan time to selected row
        const moveMin = Math.max(0, Math.abs(dt) - heldMinBetween);
        const direction = dt >= 0 ? 1 : -1;
        let frontKm = row.distKm - direction * stats.avgKmPerMin * moveMin;
    ...
    function updateSummaries(positions, row){
      ...
      if (rowSummary){
        rowSummary.innerHTML = `Selected point – ${row.dist.toFixed(1)}\nETAs at selected point\n${lines.join('\n')}`;
      }
      if (gapSummary) gapSummary.textContent = '';
    }
    ...
    function setupScanner(){
      ...
      if (total <= windowWidthKm){
        ...
        label.textContent = `Route scanner – stage is ${total.toFixed(1)} units (shorter than 30 units), full route is shown.`;
      ...
      label.textContent = `Route scanner – position: ${scannerPosKm.toFixed(1)} (centre ${windowCenterKm.toFixed(1)} of ${total.toFixed(1)})`;
    ...
    function renderTimeline(){
      ...
      for (let d = firstTick; d <= viewMaxKm + 0.001; d += 5){
        ...
        text(x, gridBottom + 14, d.toFixed(0), { anchor:'middle', size:9, fill:'#6b7280' });
      }
      text(width - 10, gridBottom + 28, 'Distance along route', { anchor:'end', size:10, fill:'#6b7280' });
      ...
      rows.forEach(r => {
        ...
        if (isATrow || isStc || isMarshal || isOC || isStartStc){
          ...
          const lbl = isATrow ? (atTxt ? ('AT ' + atTxt) : 'AT') : (isOC ? 'OC' : (isStartStc ? 'SSTC' : r.label));
      ...
      const etaX = Math.max(xRow + 8, leftLabelMaskWidth + 6);
...
if (isAT) {
      ...
      rect(0, 0, leftLabelMaskWidth, height, { rx:0, fill:'#ffffff' });
      drawLaneLabels();
